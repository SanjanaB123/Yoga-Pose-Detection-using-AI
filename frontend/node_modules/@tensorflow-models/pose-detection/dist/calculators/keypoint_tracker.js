...
var KeypointTracker = /** @class */ (function (_super) {
    ...
    KeypointTracker.prototype.area = function (keypoints) {
        ...
        var minX = Math.min.apply(Math, [1.0].concat(validKeypoint.map(function (kpt) { return kpt.x; })));
        var maxX = Math.max.apply(Math, [0.0].concat(validKeypoint.map(function (kpt) { return kpt.x; })));
        var minY = Math.min.apply(Math, [1.0].concat(validKeypoint.map(function (kpt) { return kpt.y; })));
        var maxY = Math.max.apply(Math, [0.0].concat(validKeypoint.map(function (kpt) { return kpt.y; })));
