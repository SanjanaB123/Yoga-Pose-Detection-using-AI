...
class Movenet(object):
  ...
  def _torso_visible(self, keypoints: np.ndarray) -> bool:
    # this function checks the min no of torso points 
  ...
  def _determine_torso_and_body_range(self, keypoints: np.ndarray, target_keypoints: Dict[(str, float)], center_y: float, 
  ...
  def _run_detector(
      self, image: np.ndarray, crop_region: Dict[(str, float)],
      crop_size: (int, int)) -> np.ndarray:
    """Runs model inference on the cropped region.The function runs the model inference on the cropped region and updates
     the model output to the original image coordinate system.It returns an array of shape [17, 3] representing the keypoint absolute coordinates
      and scores.
    """
